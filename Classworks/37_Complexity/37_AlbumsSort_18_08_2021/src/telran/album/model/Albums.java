...
public class Albums implements IAlbums {
	...
	@Override
	public Photo[] getPhotosByPredicate(Predicate<Photo> predicate) {
		int count = 0;
	...
	public Photo[] getPhotoBeetwenDate(LocalDate dateFrom, LocalDate dateTo) {
		...
		Photo pattern = new Photo(0, Integer.MIN_VALUE, null, null, dateFrom.atTime(LocalTime.MIN));
		int from = -Arrays.binarySearch(photos, 0, size, pattern) - 1;
		pattern = new Photo(0, Integer.MAX_VALUE, null, null, dateTo.atTime(LocalTime.MAX));
		int to = -Arrays.binarySearch(photos, 0, size, pattern) - 1;
